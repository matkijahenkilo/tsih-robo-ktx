...
fun makeTwitterCommand(link: String, shouldGetInfoOnly: Boolean = false, limit: Int = defaultLimit): List<String> =
    baseArgs + if (shouldGetInfoOnly) infoArgs(link, twitterFilters) else downloadArgs(link, limit)

fun makeMisskeyCommand(link: String, shouldGetInfoOnly: Boolean = false, limit: Int = defaultLimit): List<String> =
