...
import org.matkijahenkilo.tsihRoboKtx.utils.parseDurationToSeconds
import org.matkijahenkilo.tsihRoboKtx.utils.replaceInstantChars
import org.matkijahenkilo.tsihRoboKtx.utils.replaceLast
...
class ToolPost(private val event: GenericCommandInteractionEvent) : SlashCommand(event) {
    ...
    override fun execute() {
        if (toolPostBaseVideoExists()) {
            ...
            val link = event.getOption(TOOLPOST_OPTION_LINK)!!.asString
            val climaxTime = event.getOption(TOOLPOST_OPTION_TRIM)?.asString
