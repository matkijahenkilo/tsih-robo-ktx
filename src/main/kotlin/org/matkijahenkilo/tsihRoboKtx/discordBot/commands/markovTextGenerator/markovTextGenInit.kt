...
import org.matkijahenkilo.tsihRoboKtx.discordBot.commands.markovTextGenerator.slash.MarkovRoomHandler
...
fun markovTextGenInit(jda: JDA): SlashCommandData {
        ...
        if (savedMarkovChannels.any { it.writingChannelId == event.channel.idLong }) {
            ...
            if (Random.nextFloat() * 100 <= chance || isForced) {
                ...
                if (!sentence.isNullOrEmpty())
                    ...
                    event.channel.send(
                        content = "-# I don't know this word. Did you also run `/${MarkovRoomHandler.MARKOV}` ?"
                    ).queue()
            }
        }
    }
     ...
     */
    jda.onCommand(MarkovRoomHandler.MARKOV) { event ->
        when {
            !event.isFromGuild -> {
        ...
        if (event.subcommandName == MarkovRoomHandler.OPTION_READ) {
    ...
    return MarkovRoomHandler.getCommands()
}